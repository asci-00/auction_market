...
- The notification settings section now keeps the current OS permission state visible next to account-level push preferences and exposes the system-settings recovery action when device notification permission is blocked.
...
- Mobile now handles push payloads through `onMessage`, `getInitialMessage`, and `onMessageOpenedApp`, surfaces Android foreground messages as high-priority local system notifications with a `SnackBar` fallback, keeps iOS foreground presentation on Firebase Messaging presentation options, routes opened notifications through the existing deep-link resolver, and falls back to `/notifications` when a push deeplink is missing or unsupported.
...
- `cd apps/mobile_flutter && flutter analyze lib/features/settings/presentation/settings_screen.dart lib/features/settings/presentation/widgets/settings_notification_section.dart test/features/settings/presentation/settings_screen_test.dart test/features/settings/presentation/widgets/settings_notification_section_test.dart` passed on May 2, 2026 after surfacing notification permission state and adding debug push-probe result coverage.
- `cd apps/mobile_flutter && flutter test test/features/settings/presentation/settings_screen_test.dart test/features/settings/presentation/widgets/settings_notification_section_test.dart` passed on May 2, 2026 after surfacing notification permission state and adding debug push-probe result coverage.
- `cd apps/mobile_flutter && flutter analyze test/features/notifications/application/notification_device_token_service_test.dart` passed on May 4, 2026 after adding token lifecycle branch coverage for denied permission, token rotation, and iOS APNs-not-ready handling.
- `cd apps/mobile_flutter && flutter test test/features/notifications/application/notification_device_token_service_test.dart` passed on May 4, 2026 after adding token lifecycle branch coverage for denied permission, token rotation, and iOS APNs-not-ready handling.
- `cd apps/mobile_flutter && flutter analyze lib/features/notifications/application/notification_push_service.dart test/features/notifications/application/notification_push_service_test.dart` passed on May 6, 2026 after switching foreground push presentation to local system notifications.
- `cd apps/mobile_flutter && flutter test test/features/notifications/application/notification_push_service_test.dart` passed on May 6, 2026 after adding foreground local-notification presentation and tap-routing coverage.
- `cd apps/mobile_flutter && flutter build apk --debug` passed on May 6, 2026 after adding Android desugaring and a notification status drawable for foreground local notifications.
- `cd apps/mobile_flutter && flutter gen-l10n` passed on May 10, 2026 after simplifying utility-screen copy and adding notification inbox summary copy.
- `cd apps/mobile_flutter && dart format lib test` passed on May 10, 2026 after refreshing generated localization files.
- `cd apps/mobile_flutter && flutter analyze` passed on May 10, 2026 after enabling debug App Check providers, adding token-registration caching, and replacing utility-screen heroes with functional controls.
- `cd apps/mobile_flutter && flutter test` passed on May 10, 2026 after the notification lifecycle and utility-screen UX refinement pass.
