...
- Loading-overlay barriers, order payment sub-plates, activity stat icon tiles, and home header action buttons now resolve theme-aware nested surfaces too, so common dark-mode routes no longer mix light inset panels into otherwise warm dark screens.
- Orders payment dialogs and sheets plus auction amount dialogs now share a common modal barrier color and fade-plus-rise presentation helper, and the shared loading overlay resolves from the same barrier tuning so transactional overlays do not drift apart across Phase 3 flows.
- Auction detail hero overlays and fallback artwork, sell image fallback tiles, login support copy panels, and payment return status surfaces now resolve brightness-aware tokens too, so common dark-mode journeys no longer leak light-only panels or muted text.
