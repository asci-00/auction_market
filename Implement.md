...
- Latest mobile verification: `cd apps/mobile_flutter && flutter gen-l10n`, `dart format lib test`, `flutter analyze`, `flutter test`, and `flutter build apk --debug` passed on May 10, 2026 after the notification readiness and utility-screen UX refinement pass.
- Use the README emulator flow and the environment runbook for full reproduction steps when validating push delivery against local or real-device runtimes.
