...
} from '../src/domain/schedulerEngine.js';
...
describe('scheduler engine', () => {
    ...
    const result = finalizeAuction(
      {
        ...
        sellerId: 's1',
        status: 'LIVE',
        endAt: new Date(now.getTime() - 1_000),
        currentPrice: 10000,
        highestBidderId: 'u1',
      },
      now,
  ...
  it('builds an awaiting payment order with nested payment and fee shape', () => {
    const now = new Date();
    ...
    expect(order.payment.provider).toBe('TOSS_PAYMENTS');
    expect(order.payment.paymentKey).toBeNull();
    expect(order.fees.sellerReceivable).toBeGreaterThan(0);
  ...
  it('settles only when expectedAt passed', () => {
    const now = new Date();
    expect(
      shouldSettle('CONFIRMED_RECEIPT', new Date(now.getTime() - 1000), now),
    ).toBe(true);
    expect(
      shouldSettle('CONFIRMED_RECEIPT', new Date(now.getTime() + 1000), now),
    ).toBe(false);
