...
describe('auction engine', () => {
  it('extends auction by 5 minutes near end, max 3', () => {
    ...
    let auction: any = {
      id: 'a1', itemId: 'i1', sellerId: 's1', startPrice: 10000, currentPrice: 10000,
      status: 'LIVE', endAt: baseEnd, extendedCount: 0, bidCount: 0, bidderCount: 0,
    };

    for (let i = 0; i < 4; i += 1) {
      const result = placeBid({ auction, bidderId: `u${i}`, amount: auction.currentPrice + 1000, now: new Date() });
  ...
  it('auto-bid competition rises and determines highest bidder', () => {
    ...
    const auction: any = {
      id: 'a1', itemId: 'i1', sellerId: 's1', startPrice: 10000, currentPrice: 10000,
      status: 'LIVE', endAt: new Date(Date.now() + 60 * 60 * 1000), extendedCount: 0, bidCount: 0, bidderCount: 0,
    };
