...
- `dart_defines.local-emulator.json`은 dev flavor를 유지한 채 동일한 HTTP backend contract를 로컬 API URL로 연결하고, `USE_FIREBASE_EMULATORS=true`로 Firebase Auth/Firestore/FCM을 로컬 에뮬레이터에 바인딩합니다.
